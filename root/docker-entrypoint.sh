...
USERID="${PUID:-1000}"
GROUPID="${GUID:-1000}"
useradd -M -s /sbin/nologin -u $USERID -U -d /nonexistent monit || die "Failed to add user monit with PUID:$USERID and group with GUID:$GROUPID"

### Setup logging
chown monit:monit /run
